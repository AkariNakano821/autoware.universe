...
bool StopLineModule::modifyPathVelocity(PathWithLaneId * path, StopReason * stop_reason)
{
  ...
  *stop_reason = planning_utils::initializeStopReason(StopReason::STOP_LINE);
  ...
  auto stop_pose = trajectory->compute(stop_point_s);
   ...
   * s---ego----------x--|--------g
   */
  double ego_on_trajectory_s = trajectory->closest(planner_data_->current_odometry->pose.position);
  double signed_arc_dist_to_stop_point = stop_point_s - ego_on_trajectory_s;
  ...
  path->points = trajectory->restore(3);
