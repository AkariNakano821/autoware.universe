...
def get_random_memory_bank(X):
    ...
    return memory_bank
def get_acc_steer_nominal_prediction(X,window_size=10):
...
def get_nominal_signed_prediction_error(X,window_size=10):
    nominal_prediction = get_acc_steer_nominal_prediction(X,window_size)
...
def plot_relearned_vs_original_prediction_error(
        ...
        if plt_save_dir is not None:
            ...
            for i in range(len(state_component_predicted)):
                ...
                axes[i,1].legend()
                if Y_nominal_test_pred is not None:
